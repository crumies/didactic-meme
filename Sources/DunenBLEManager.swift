...
final class DunenBLEManager: NSObject, ObservableObject {
    ...
    private func calculateDerived(dt: Double) {
        ...
        // Better Aptum speed estimate:
        // real reference from bike: Sports mode 8000 RPM ≈ 136 km/h.
        // 136 / 8000 = 0.017 km/h per motor RPM.
        let rpmSpeedKmh: Double = {
            if telemetry.mode == .reverse {
                return Double(max(0, telemetry.rpm)) * 8.5 / 300.0
            }
            return Double(max(0, telemetry.rpm)) * 136.0 / 8000.0
        }()

        // If controller speed is missing, too low, or clearly unrealistic vs RPM, trust RPM.
        // If controller speed looks valid, blend it with RPM to avoid jumping.
        if telemetry.rpm > 150 {
            let controllerSpeed = telemetry.speedKmh
            if controllerSpeed <= 1 || controllerSpeed > 160 || abs(controllerSpeed - rpmSpeedKmh) > 28 {
                telemetry.speedKmh = rpmSpeedKmh
            } else {
                telemetry.speedKmh = controllerSpeed * 0.65 + rpmSpeedKmh * 0.35
            }
        }
        ...
        // Estimated total motor-to-wheel reduction from your real reference.
        // 136 km/h at 8000 motor RPM with 18 inch rear wheel ≈ 2.88:1 total ratio.
        let estimatedTotalRatio = max(1.0, Double(max(1, telemetry.rpm)) / max(1.0, telemetry.wheelRPM))
        telemetry.wheelTorqueNm = telemetry.torque * estimatedTotalRatio * 0.88
